...
GLfloat ambientColor0[] = { 0.5, 0.5, 0.8, 0.4 };//Глобальный свет(свет отражения от объектов) 
GLfloat diffuseColor0[] = { 0.4, 0.2, 0.7, 0.7 };//Свет в сферу(освещение сферы)
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Джалолов_Алишер_61гр.");
...
void myDisplay()
{
	...
	glLightf(GL_LIGHT1, GL_SPOT_CUTOFF, 35.0);// Угол конуса
	glLightf(GL_LIGHT1, GL_SPOT_EXPONENT, 10.0);//Блюр
	
	glPushMatrix(); 			//запоминаем текущую матрицу
		//видовые параметры
	glTranslatef(1.0, 0.0, 0.0);		//начальный сдвиг системы координат
	glRotatef(35.0, 9.0, 8.0, 6.0);		//поворот относительно оси X
	glRotatef(0.0, 0.0, 0.0, 0.0);		//поворот относительно оси Y
	glBegin;
	glutSolidCube(350);		// команды рисования
		glEnd;
	glPopMatrix();
